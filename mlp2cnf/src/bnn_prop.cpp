...
/*
 * This has been refactored into BNNPropertyEncoder
 */
DifferentialNN::DifferentialNN(string bnn1_model_dir, string bnn2_model_dir, string out_filename) {
	this->model_dir = model_dir;
	...
	this->bnn1 = new BNNModel(bnn1_model_dir, out_dir);
	this->bnn2 = new BNNModel(bnn2_model_dir, out_dir);
}

int DifferentialNN::encode() {
	int reserved_vars;
	ofstream out_file;

	int bnn1_out_start = bnn1->blocks[0].in_size + 1;
	...
	reserved_vars = bnn1->blocks[0].in_size + bnn1->out_blk.out_size + bnn1->out_blk.lin_weight[0].size();

	// reserve vars for each of bnn1's layer's intermediate variables
	for (int i = 0; i < bnn1->num_internal_blocks; i++)
		reserved_vars += bnn1->blocks[i].out_size;

	int bnn2_interm_var_start = reserved_vars + 1;

	// this is where the bnn2's intermediate variables start
	cout << "bnn2 intermediate vars start = " << bnn2_interm_var_start << endl;

	// account for bnn2
	for (int i = 0; i < bnn2->num_internal_blocks; i++)
		reserved_vars += bnn2->blocks[i].out_size;

	reserved_vars += bnn2->out_blk.out_size + bnn2->out_blk.lin_weight[0].size() + 1;
	cout << "reserved_vars for both the bnns " << reserved_vars << endl;

	int first_fresh_var = bnn1->encode(reserved_vars, 1);

	cout << "first_fresh_var = " << first_fresh_var << endl;

	int biggest_aux_var = bnn2->encode(first_fresh_var, 1, bnn2_interm_var_start) + 1;

	int bnn2_out_start = bnn2->out_blk.out_var_start;

	cout << "BNN1 OUT VAR START = " << bnn1_out_start << endl;
	cout << "BNN2 OUT VAR START = " << bnn2_out_start << endl;
	vector<vector<int>> ineq_clauses;
	int total_clauses = 0;
	// do the O1 =! O2 encoding
	// will introduce a new variable for each O1 =! O1'
	for (int i = 0; i < bnn1->out_blk.lin_weight[0].size(); i++) {
	...
	if (out_file.is_open()) {
		...
		for (int i = 0; i < bnn1->num_internal_blocks; i++)
			for (auto clause : bnn1->blocks[i].cnf_formula) {
				for (auto lit : clause)
					out_file << lit << " ";
				out_file << "0" << endl;
			}

		for (auto clause : bnn1->out_blk.cnf_formula) {
			for (auto lit : clause)
				out_file << lit << " ";
			out_file << "0" << endl;
		}
		...
		for (int i = 0; i < bnn2->num_internal_blocks; i++)
			for (auto clause : bnn2->blocks[i].cnf_formula) {
				for (auto lit : clause)
					out_file << lit << " ";
				out_file << "0" << endl;
			}

		for (auto clause : bnn2->out_blk.cnf_formula) {
			for (auto lit : clause)
				out_file << lit << " ";
			out_file << "0" << endl;
		}
