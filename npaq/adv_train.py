...
class AdvTrain(object):
    ...
    def get_batch(self, data, target):
        ...
        if len(adv_batch_data) == 0:
            final_data = origin_data
            final_label = origin_target
        else:
            adv_batch_data = adv_batch_data.reshape(adv_batch_data.shape[0],
                                                    origin_data.shape[1],
                                                    origin_data.shape[2],
                                                    origin_data.shape[3])
            final_data = torch.cat([origin_data, adv_batch_data], dim = 0).type(torch.float32)
            final_label = torch.cat([origin_target, adv_batch_target], dim = 0)
    ...
    def train(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            ...
            data, target = self.get_batch(data, target)
            info[batch_idx] = [data, target]
            sample_size += data.shape[0]
        log_str = 'Train Epoch: %d\t#Samples: %d/%d' % (epoch, sample_size,
                                                        len(self.train_loader.dataset))
        print(log_str)
        logger.debug(log_str)

        for batch_idx in info:
            data = info[batch_idx][0]
            target = info[batch_idx][1]
    ...
    def test(self, epoch=0):
        ...
        log_str = '\n[ADV TRAIN] Overall TEST: Average loss: %.4f, Acc: %d/%d (%.4f %%)' % (
