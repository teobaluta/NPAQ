...
def invoke_scalmc(formula_fname, proj_size, epsilon=0.8, delta=0.2,
                  timeout=1000, num_samples=10, realtime=True):
    """
    """
    ...
    call_args = [definitions.COUNTER, '-s', '0',
                '--samples', str(num_samples), '--sampleout', samples_fname,
                 formula_fname]
    ...
    logger.debug('scalmc witnesses in {}'.format(samples_fname))
